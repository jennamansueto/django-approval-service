...
class DeliverableViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return Response({
                ...
                'message': smart_str(_('Deliverables retrieved successfully')),
            })
        return response
    ...
    def submit(self, request, pk=None):
        ...
        if deliverable.status != Deliverable.DRAFT:
            error_msg = _('Only draft deliverables can be submitted')
            return Response(
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            message = _('Deliverable "%(title)s" submitted for approval') % {
